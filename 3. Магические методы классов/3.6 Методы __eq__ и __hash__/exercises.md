...
## ex_10
### Объявите класс с именем _Triangle_, объекты которого создаются командой:

```python
tr = Triangle(a, b, c)
```

где _a_, _b_, _c_ - длины сторон треугольника (числа: целые или вещественные). В классе _Triangle_ объявите следующие дескрипторы данных:

_a_, _b_, _c_ - для записи и считывания длин сторон треугольника.
...
Также нужно проверять, что все три стороны _a_, _b_, _c_ могут образовывать стороны треугольника. То есть, должны выполняться условия:
...
Наконец, с объектами класса _Triangle_ должны выполняться функции:

**len(tr)** - возвращает периметр треугольника, приведенный к целому значению с помощью функции _int()_;

**tr()** - возвращает площадь треугольника (можно вычислить по формуле Герона: _s = sqrt(p * (p-a) * (p-b) * (p-c))_, где _p_ - полупериметр треугольника).

P.S. На экран ничего выводить не нужно, только объявить класс _Triangle_.
